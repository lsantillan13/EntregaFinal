window.onload = function(){ // => When document is ready...
  ...
  const handleSubmit = async(e) => {
    ...
    try{
      ...
      await axios.post('https://project-chouse.herokuapp.com/api/productos', { title, price, img }, config);
  ...
  (async function(){
    try{
      //const data = await axios.get('https://project-chouse.herokuapp.com/api/productos');
  ...
  (async function(){
    /*Vars*/
    let table = [];
    //let response = await axios.get('https://project-chouse.herokuapp.com/api/mensajes');
    ...
    /*Functionality*/
    for (let i = 0; i < Data.length; i++){let content = { id: Data[i]._id, author: Data[i].author, message: Data[i].message, date: Data[i].date }; table.push(content); }
    table.forEach(e => {
      output.innerHTML += `<p> <strong>${e.author}</strong> <i class="notItalic">${e.date}</i> : <i>${e.message}</i> </p>`;
    });
    
  })(); // =>  Normalizr

  
  message !== '' && userEmail !== '' && btn.addEventListener('click', async function() {
    let author =  { id: userEmail.value, name: 'userName', lastName: 'userLastName', age: 'userAge', nick: 'userNick', avatar: 'userAvatar'};
    let Mensaje = { message: message.value, date: getTheDate()};
    ...
    try{
      let author = document.querySelector('#userEmail').value;
      let message = document.querySelector('#message').value; 
      let date = getTheDate();
      await axios.post('http://localhost:8080/api/mensajes', { author, message, date }, config);
      //await axios.post('https://project-chouse.herokuapp.com/api/mensajes', { author, message, date }, config);
    }
